...
app.post('/api/auth/sign-up', async (req, res, next) => {
  try {
    ...
    const result = await db.query(sqlNewUser, params);
...
app.post('/api/auth/sign-in', async (req, res, next) => {
  try {
    ...
    const result = await db.query(sqlExistingUser, params);
    const user = result.rows[0];
    if (!user) throw new ClientError(401, 'invalid login');

    const passwordVerify = await argon2.verify(user.hashedPassword, password);
    if (!passwordVerify) throw new ClientError(401, 'invalid login');
    ...
    const signedToken = jwt.sign(payloadObject, hashKey);
    if (!signedToken) throw new ClientError(404, 'cannot log in');
